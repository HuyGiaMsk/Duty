...
def download_source():
    ...
    download_url = f"https://github.com/HuyGiaMsk/Duty/archive/main.zip"
    print("Start download source")
    response = requests.get(download_url, verify=False)

    if response.status_code == 200:
        destination_directory = os.path.expanduser("~")
        file_name = os.path.join(destination_directory, "Duty.zip")
        ...
        os.rename(os.path.join(destination_directory, 'Duty-main'),
